...
The workflows use OIDC authentication. Ensure you have:

- `TERRAFORM_DEPLOY_ROLE` variable set in your GitHub repository (Actions → Variables)
- OIDC configured in AWS IAM for GitHub Actions

Alternatively, you can modify the workflows to use AWS access keys if needed.

### 2. Update Configuration

Edit `modules/signals-demo/terraform.tfvars`:

```hcl
demo_domain = "your-domain.com"      # A domain you control
alert_email = "alerts@your-org.com"  # For CloudWatch alarms
```
...
**Option A: Deploy from module directory** (matches workflow pattern)
```bash
cd modules/signals-demo
# Provider configuration is inherited from root or set via AWS_PROFILE/AWS_REGION env vars
...
Note: When running standalone, ensure AWS credentials are configured (via `aws configure`, environment variables, or IAM role).

**Option B: Deploy from root** (manage everything together)
```bash
# Enable the module in root terraform.tfvars
echo 'enable_signals_demo = true' >> terraform.tfvars

# Or pass as variable
terraform plan -var="enable_signals_demo=true"
terraform apply -var="enable_signals_demo=true"
```

**Recommendation**: For demos, use Option B and enable it in root `terraform.tfvars` so you can manage all infrastructure from one place. The workflows will still work independently.
...
Update your domain's nameservers at your registrar to these values.

### 5. Bootstrap Historical Pattern

**Important**: Overmind needs to see a pattern of routine changes before it can detect anomalies. Run the routine workflow manually a few times to establish this pattern:

1. Go to **Actions** → **Routine Customer IP Update**
2. Click **Run workflow**
3. Set `rotation_override` to `0`, run
4. Wait for it to complete (~2-3 minutes)
5. Repeat with values `1`, `2`, `3` (one at a time, waiting for each to complete)

**Why this matters**: Overmind builds a baseline of "normal" changes. Without this history, it won't recognize that `customer_access` SG changes are routine vs. `internal_services` SG changes being unusual.

**Alternative**: Just wait—the scheduled job runs Mon/Wed/Fri at 2am UTC automatically, but you'll need to wait several days for enough history.
